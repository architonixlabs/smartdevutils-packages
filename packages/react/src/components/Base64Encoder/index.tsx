...
export function Base64Encoder({ onResult }: Base64EncoderProps) {
  ...
  const process = (value: string, m: 'encode' | 'decode') => {
    ...
    setError('')
    try {
      const result = m === 'encode' ? base64Encode(value) : base64Decode(value)
      setOutput(result)
      onResult?.(result, m)
    } catch {
      setError(m === 'decode' ? 'Invalid Base64 input' : 'Encoding failed')
      setOutput('')
    }
  }

  return (
    <ToolPanel title="Base64 Encoder / Decoder">
      <div className="space-y-4">
        ...
        <textarea value={input} onChange={e => process(e.target.value, mode)}
          placeholder={mode === 'encode' ? 'Enter text to encode...' : 'Enter Base64 to decode...'} rows={4}
