...
import { cssFormat, cssMinify } from '@smartdevutils/core/formats'
import { CopyButton, ErrorDisplay, ToolPanel } from '../shared'

export function CssFormatter({ onResult }: { onResult?: (r: string) => void }) {
  ...
  const process = (value: string, action: 'format' | 'minify') => {
    ...
    try {
      const r = action === 'format' ? cssFormat(value) : cssMinify(value)
      setOutput(r); onResult?.(r)
    } catch { setError('Invalid CSS'); setOutput('') }
  }

  return (
    <ToolPanel title="CSS Formatter">
      <div className="space-y-4">
        ...
        <textarea value={input} onChange={e => setInput(e.target.value)} placeholder=".class { color: red; }" rows={8}
