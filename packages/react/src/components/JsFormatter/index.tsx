...
import { jsFormat, jsMinify } from '@smartdevutils/core/formats'
import { CopyButton, ErrorDisplay, ToolPanel } from '../shared'

export function JsFormatter({ onResult }: { onResult?: (r: string) => void }) {
  ...
  const process = (value: string, action: 'format' | 'minify') => {
    ...
    try {
      const r = action === 'format' ? jsFormat(value) : jsMinify(value)
      setOutput(r); onResult?.(r)
    } catch { setError('Failed to process JavaScript'); setOutput('') }
  }

  return (
    <ToolPanel title="JavaScript Formatter">
      <div className="space-y-4">
        ...
        <textarea value={input} onChange={e => setInput(e.target.value)} placeholder="const x = 1" rows={8}
